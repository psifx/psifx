...
class StreamSeparationTool(BaseTool):
    def __init__(
        self,
    ...
    def __call__(
        ...
        if not isinstance(out_video_path, Path):
        ...
        (
            ...
            .video.output(str(out_video_path), **{"vcodec": "copy"})
            .overwrite_output()
...
def cli_main():
    ...
    if args.in_video.is_file():
        ...
        for in_video_path in sorted(in_video_dir.glob("*")):
            out_video_name = in_video_path.stem + ".mp4"
